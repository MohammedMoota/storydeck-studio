...
const GitHubButton = () => {
  return (
    <button
      onClick={() =>
        window.open("https://github.com/MohammedMoota/storydeck-studio", "_blank")
      }
      className="flex items-center gap-2 px-3 py-2 hover:bg-gray-100 rounded-lg transition-colors cursor-pointer"
      style={{ width: "100%" }}
    >
      <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor">
        <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82.22-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z" />
      </svg>
      {
        //<GitHubStars repo="mohammedmoota" />
      }
    </button>
  );
};
...
export default function Canvas() {
      ...
      <div className="fixed inset-0 bg-white/80 backdrop-blur-md flex items-center justify-center p-6 z-50">
        <div className="bg-white p-8 rounded-3xl shadow-2xl max-w-md w-full text-center border border-gray-200">
          ...
          <p className="text-gray-600 leading-relaxed">
            StoryDeck is designed for larger screens to give you the best creative experience. Please switch to a tablet, laptop, or desktop computer to continue.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <FrameGraphProvider editor={editor}>
      <div style={{ position: "fixed", inset: 0 }} className="bg-white">
        {/* Prismatic Overlay for Canvas */}
        <div className="absolute inset-0 pointer-events-none z-0 opacity-20 bg-[radial-gradient(circle_at_50%_50%,rgba(255,154,158,0.1),rgba(255,255,255,0))]" />
        
        <Tldraw
          onMount={handleEditorMount}
          shapeUtils={customShapeUtils}
          persistenceKey="storydeck-canvas-v1"
          hideUi={hideUi}
          components={{
            OnTheCanvas: () => (
              <>
        ...
        options={{ maxPages: 1 }}
          // Removed tldraw-mode-dark to allow light mode
          className="tldraw-mode-light"
        ></Tldraw>
