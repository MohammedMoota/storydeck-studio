      ...
      {info.map((arrowInfo) => {
        ...
        if (arrowInfo.status === "pending") {
          return (
            ...
            >
              <div
                className="flex items-center justify-center px-6 py-4 bg-black/80 backdrop-blur-md rounded-xl shadow-2xl border border-white/20"
                style={{ minWidth: "220px", whiteSpace: "nowrap" }}
              >
                <Brain className="w-6 h-6 text-brand-purple animate-pulse" />
                <span className="ml-3 font-bold text-xl bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">
                  Dreaming... {arrowInfo.timer || 0}s
                </span>
              </div>
            </div>
          );
